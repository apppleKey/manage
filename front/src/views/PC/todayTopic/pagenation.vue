<template>
  <!--分页组件-->
  <div class="pageComponent" v-if="pageConfig.pageTotal>1 &&isInited">
      <!--上一页-->
...
<script>
  export default {
    ...
    watch:{
        $props:{
            handler:function(newV ,oldV){
                this.currentPage=newV.pageConfig.pageNo;
                console.log('watcher---',newV.pageConfig.pageTotal)
            },
            immediate:true,
            deep:true
        }
    ...
    methods: {
      ...
      jumpTo(){
           if(this.jumpPage && /[1-9][0-9]*/.test(this.jumpPage)){             
                var pg = parseInt(this.jumpPage)
                if(pg > 0 && pg <= this.pageTotal){
                    this.$emit('changeCurrentPage', pg ) //调用父组件方法
                }else{
                    this.jumpPage = ""
                }
            }else{
...
<style lang="scss">
  .pageComponent {
    ...
    input{
        width: 50px;
        height: 28px;
        border: 1px solid #fe566156;
        padding-left: 3px;
        &:focus{
            border:1px solid #fe5661;
             outline: none;
